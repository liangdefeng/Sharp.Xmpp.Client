...
namespace Sharp.Xmpp.Extensions
{
    ...
    /// <summary>
    /// Implements Direct MUC Invitation as described in XEP-0249.
    /// </summary>
    public class DirectMucInvite : Message
    {
        ...
        public DirectMucInvite(Jid to, Jid from, Jid room, string reason, string password = null)
            : base(room, from, Xml.Element(xTag, conference))
        {
            XElement.Child(Xml.Element(jidTag).Child(Xml.Element(reasonTag)));
            SendTo = to;
            Reason = reason;
            Password = password;
        }
        ...
        private XmlElement InviteElement { get { return GetNode(xTag, jidTag); } }

        private XmlElement ReasonElement { get { return GetNode(xTag, jidTag, reasonTag); } }
